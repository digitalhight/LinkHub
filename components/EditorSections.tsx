  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-6">
        <div className="relative w-20 h-20 lg:w-24 lg:h-24 rounded-3xl overflow-hidden border border-white/10 group bg-white/5 mx-auto lg:mx-0">
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Nom complet</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Pseudo (@)</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Biographie</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">WhatsApp / Phone</label>
              <input type="tel" value={profile.phone} onChange={(e) => setProfile({...profile, phone: e.target.value})} placeholder="+33..." className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-white outline-none focus:border-purple-500/50 transition-all" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Email Contact</label>
              <input type="email" value={profile.email} onChange={(e) => setProfile({...profile, email: e.target.value})} placeholder="contact@..." className="w-full px-5 py-4 bg-white/5 border border-white/10 rounded-2xl text-sm font-bold text-white outline-none focus:border-purple-500/50 transition-all" />
            </div>
          </div>
        </div>
      </div>
...
export const LinksSection: React.FC<{ profile: UserProfile, setProfile: (p: UserProfile) => void }> = ({ profile, setProfile }) => {
  const addLink = () => {
    const newLink: LinkItem = { id: Date.now().toString(), title: 'Nouvel Atout', url: 'https://', isActive: true };
  ...
  return (
    <div className="space-y-6 lg:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <button onClick={addLink} className="w-full py-4 lg:py-5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-3 hover:scale-[1.02] transition-all shadow-xl active:scale-95">
        <Icons.Plus /> Ajouter un Atout
      </button>

      <div className="space-y-4">
        {profile.links.map((link) => (
          <div key={link.id} className="bg-white/5 border border-white/5 rounded-[1.5rem] lg:rounded-[2rem] p-5 lg:p-6 group hover:bg-white/[0.08] transition-all">
...
export const ThemeSection: React.FC<{ profile: UserProfile, setProfile: (p: UserProfile) => void }> = ({ profile, setProfile }) => {
  return (
    <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-2 gap-4 animate-in fade-in zoom-in duration-500">
      {DEFAULT_THEMES.map(t => (
        <button 
          key={t.id} 
          onClick={() => setProfile({...profile, theme: t})}
          className={`aspect-[4/5] rounded-[1.2rem] lg:rounded-[1.5rem] border-2 transition-all overflow-hidden relative group ${profile.theme.id === t.id ? 'border-purple-500 shadow-2xl shadow-purple-500/20' : 'border-white/5 hover:border-white/10'}`}
        >
          <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${t.backgroundStart}, ${t.backgroundEnd})` }}></div>
          <div className="absolute inset-x-0 bottom-0 p-3 lg:p-4 bg-black/40 backdrop-blur-md">
            <p className="text-[8px] lg:text-[9px] font-black text-white uppercase tracking-widest truncate">{t.name}</p>
          </div>
          {profile.theme.id === t.id && (
            <div className="absolute top-3 right-3 bg-purple-500 text-white p-1 rounded-full shadow-lg"><Icons.Check /></div>
          )}
        </button>
      ))}
    </div>
