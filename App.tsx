...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Filtrer les segments vides et ignorer les mots clés système ou protocoles
  ...
  const fetchProfile = async (id: string) => {
    try {
      ...
      console.warn('Profile fetch error ignored');
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Erreur : ${err.message || 'La sauvegarde a échoué. Vérifiez vos colonnes Supabase.'}`);
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-white z-50">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em]">Chargement</p>
        </div>
      </div>
    );
  }
